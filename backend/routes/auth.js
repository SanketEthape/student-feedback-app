...
router.post('/register', async (req, res) => {
    try {
        ...
        const token = jwt.sign({ id: faculty._id, role: 'faculty' }, process.env.JWT_SECRET, { expiresIn: '7d' });
...
router.get('/me', auth, async (req, res) => {
    ...
    // Return hasApiKey flag (don't expose the key itself)
    res.json({
        ...faculty.toObject(),
        hasApiKey: !!faculty.geminiApiKey,
...
router.put('/api-key', auth, async (req, res) => {
    try {
        ...
        await Faculty.findByIdAndUpdate(req.faculty.id, { geminiApiKey: encryptedKey });
